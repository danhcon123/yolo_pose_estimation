...
def get_device() -> str:
    ...
    Priority: CUDA GPU > Apple Silicon GPU (MPS) > CPU.
    """
...
def get_gpu_info() -> str | None:
    """
    Return a readable description of the GPU that will be used,
    or None if no supported GPU is available.
    """
    if torch.cuda.is_available():
        ...
        info = (
            f"{name} "
            f"({vram_gb:.1f} GB VRAM, CUDA {cuda_version})"
        )

        if device_count > 1:
            info += f" [{device_count} GPUs detected, using GPU {device_index}]"
...
def save_chart(
    ...
    ax.set_title(
        f"{device_label} inference time comparison of YOLO pose models"
    )
...
def main() -> None:
    ...
    if gpu_info is not None:
        ...
        print("GPU:          None available (falling back to CPU)")
